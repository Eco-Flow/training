...
<img src="img/seqera_access_tokens.png" alt="The Access tokens page in Seqera Platform, with the Add token button at the top right" width="700"/>
...
Add **`-with-tower`** to any `nextflow run` command. Let's use the same small pipeline as [Part 7](./hpc.md):
...
| :--- | :--- |
| **Tasks** | Every task, with its status (pending, running, cached, succeeded, failed) and the resources it used. Click one to see its command, work directory and logs — the same things you dug out of `work/` by hand in [Part 3](./nfcore_rnaseq.md). |
...
It's the same flag. In the driver job script from [Part 7 Step 5](./hpc.md), add the token and `-with-tower`:
...
- Need to set up Nextflow on a cluster with no config yet? See ★ **[Advanced: setting up Nextflow for your HPC](./hpc_config.md)**.
