...
🧭 [◀️ Part 7 · GitHub](./github_basics.md) &nbsp;|&nbsp; [🏠 Course menu](../README.md) &nbsp;|&nbsp; **Next:** [Part 9 · Seqera Platform ▶️](./seqera_platform.md)
...
> ⚠️ **Always pin the version** (see [Part 3](./nfcore_rnaseq.md)). Nextflow itself changes too, and a very old pipeline release may not run on a current Nextflow. If an old release stops with `Config parsing failed`, move `-r` to a newer release.
...
>
> Nextflow writes a job script for every task: the `.command.run` file you met in [Part 3](./nfcore_rnaseq.md). Take the hash from a **FASTQC** line of *your* output and look at the top of its script:
>
> ```bash
...
You rarely write this yourself: if your cluster is listed on [nf-co.re/configs](https://nf-co.re/configs), its profile already contains it (Step 6). Writing one from scratch is the ★ [advanced page](./hpc_config.md).
</details>
