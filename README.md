...
> 🚧 **Coming soon — Part 4 · [Differential expression](./eco-flow-training/docs/differential.md)** (Practical · R): analyse the gene counts from Part 3 with DESeq2 to find differentially expressed genes. *(Draft — still in progress.)*

> 🟣 **Bonus — [Running a pipeline on an HPC](./eco-flow-training/docs/hpc.md)** (optional): how to configure and run the pipeline on a Slurm/SGE cluster. Most learners can skip this.
